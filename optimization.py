...
function = sym.expand((x1 + 10*x2)**2 + 5*(x3 - x4)**4 + (x2 - 2*x3)**4 + 10*(x1-x4)**4)
...
def proj_grad_desc(func, pf, var, start, eps, s=0.2):
    ...
    while norm(grad_subs(diff, xk)) > eps:
        gradient_vec = grad_subs(diff, xk)
...
if __name__ == "__main__":
    print(newton(function, variables, [3, -1, 0, 1], 0.01))
